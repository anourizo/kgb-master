...
Options[DefDerivedTensorsSVT] = Join[{
	MaxTimeDerivatives -> 6,(* For GR should be 4*) (* For KGB should be 6*)
...
SVTDecomposition[expr_, orderPT_, freeindsrules_, opts : OptionsPattern[{SVTDecomposition, GlobalOptionsSVT}]] := Module[
	...
	If[runGRToBuildingBlocks,
		...
		(* checked this to see GRToBuildingBlocks works *)
		(* tmpexpr = tmpexpr// ReplaceDummies // ScreenDollarIndices; *)
		(* Print[tmpexpr]; *)  
		time2 = SessionTime[];
		If[verbose, xSVTUtilities`PrintLevel["Done! Evaluated module in "<>ToString[time2-time1]<>" seconds.", 1]];,
	...
	If[runSplitSpaceTime,
		...
		(* checked this to see SplitSpaceTime works *)
		(* tmpexpr = tmpexpr// ReplaceDummies // ScreenDollarIndices; *)
		(* Print[tmpexpr]; *)
		time2 = SessionTime[];
		If[verbose, xSVTUtilities`PrintLevel["Done! Evaluated module in "<>ToString[time2-time1]<>" seconds.", 1]];,
	...
	If[storeresults,
		...
		Module[{
			...
			timevecs = Casesnow [timevecs,a_:>timevec[a]];
			timevecs = timevecs /.List->Times;
...
NOrder[n_][expr_]:=Module[{tmp,orderTder,orderSder,orderpert,nspert,nvpert,ntpert,nspertpow,nvpertpow,ntpertpow,order},tmp=expr//Expand;
tmp=Replace[tmp,Plus->List,{1},Heads->True];
If[ToString[Head[tmp]]!="List",tmp={tmp}];
...
nspertpow=Cases[#,Power[_[LI[_]],pow_]:>pow,Infinity]&/@tmp//.List[]:>List[1];
nvpertpow=Cases[#,Power[_[LI[_],_],pow_]:>pow,Infinity]&/@tmp//.List[]:>List[1];
ntpertpow=Cases[#,Power[_[LI[_],_,_],pow_]:>pow,Infinity]&/@tmp//.List[]:>List[1];
