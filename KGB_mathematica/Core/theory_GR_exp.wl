...
(*Creating Hide and Show Cell options and loading MaTeX*)


(* ::Input:: *)
(*SetOptions[EvaluationNotebook[],DockedCells->Cell[BoxData[ToBoxes[Row[{Button["Hide Cell",With[{nb=SelectedNotebook[]},SelectionMove[nb,All,Cell];*)
(*SetOptions[NotebookSelection[nb],CellOpen->False]],Method->"Queued"],Spacer[10],Button["Show Cell",With[{nb=SelectedNotebook[]},SelectionMove[nb,All,Cell];*)
(*SetOptions[NotebookSelection[nb],CellOpen->True]],Method->"Queued"]}]]],"DockedCell"]]*)


(* ::Input:: *)
(*Needs["MaTeX`"] (* load the package*)*)
...
(* Defining 3D metric named \delta *)
...
(* This rule sets the partial derivative of the flat metric tensor components to zero,
 reflecting that the flat metric is constant and does not change across the manifold. *)
...
(* the contraction of timevec with itself (over indices of manifold M1) equals 1, suggesting timevec is a unit vector *)
	timevec[-a_?TangentM1`Q] timevec[a_?TangentM1`Q] :> 1, 
(* components of timevec associated with any index from manifold M3 are zero indicating timevec has no components in the dimensions described by M3 *)
	timevec[i_?TangentM3`pmQ] :> 0,
(* the partial derivative of timevec with respect to any index is zero, implying that timevec is constant across the manifold it is defined on *)
...
DefConstantSymbol[GN, PrintAs->"\!\(\*SubscriptBox[\(G\), \(N\)]\)"]
...
DefTensorSVT[pertchi[LI[order]], {M1, M3}, PrintAs->"\!\(\*StyleBox[\"\[Chi]\",\nFontSize->14]\)", ScalarPertQ->True]
chirule=pertpsi[LI[ord_]]:>pertphi[LI[ord]]-pertchi[LI[ord]]

DefTensorSVT[pertS[LI[order]], {M1, M3}, PrintAs -> "S", ScalarPertQ->True]
DefTensorSVT[pertE[LI[order]], {M1, M3}, PrintAs -> "E", ScalarPertQ->True]


DefTensorSVT[pertB[LI[order], -i], {M1, M3}, PrintAs -> "B", VectorPertQ->True]
...
(*metricrules = {
	...
	metricg[a_?TangentM1`Q, b_?TangentM1`Q] :> -timevec[a] timevec[b] scale[]^-2, 
	metricg[i_?TangentM3`Q, j_?TangentM3`Q] :> scale[]^-2 metric\[Delta][i, j], 
	pertmetricg[LI[order_], -a_?TangentM1`Q, -b_?TangentM1`Q] :>  scale[]^2(-Exp[ 2 pertpsi[LI[order]]] timevec[-a] timevec[-b] +timevec[-a] timevec[-b]), 
	pertmetricg[LI[order_], -a_?TangentM1`Q, -i_?TangentM3`Q] :> timevec[-a] scale[]^2 (PD[-i]@pertS[LI[order]] - pertB[LI[order], -i]), 
	pertmetricg[LI[order_], -i_?TangentM3`Q, -a_?TangentM1`Q] :> timevec[-a] scale[]^2 (PD[-i]@pertS[LI[order]] - pertB[LI[order], -i]), 
	pertmetricg[LI[order_], -i_?TangentM3`Q, -j_?TangentM3`Q] :> 
		scale[]^2 (Exp[-2 pertphi[LI[order]]] metric\[Delta][-i, -j] -metric\[Delta][-i, -j] + perth[LI[order], -i, -j] + PD[-i]@pertF[LI[order], -j] + PD[-j]@pertF[LI[order], -i] + 2 PD[-i]@PD[-j]@pertE[LI[order]]),
	Detmetricg[] :> -scale[]^8};
If[fill,
...
metricrules = {
	...
	metricg[a_?TangentM1`Q, b_?TangentM1`Q] :> -timevec[a] timevec[b] scale[]^-2, 
	metricg[i_?TangentM3`Q, j_?TangentM3`Q] :> scale[]^-2 metric\[Delta][i, j], 
	pertmetricg[LI[order_], -a_?TangentM1`Q, -b_?TangentM1`Q] :>  scale[]^2( -(2 pertpsi[LI[order]]+2*pertpsi[LI[order]]^2) timevec[-a] timevec[-b] ), 
	pertmetricg[LI[order_], -a_?TangentM1`Q, -i_?TangentM3`Q] :> timevec[-a] scale[]^2 (PD[-i]@pertS[LI[order]] - pertB[LI[order], -i]), 
	pertmetricg[LI[order_], -i_?TangentM3`Q, -a_?TangentM1`Q] :> timevec[-a] scale[]^2 (PD[-i]@pertS[LI[order]] - pertB[LI[order], -i]), 
	pertmetricg[LI[order_], -i_?TangentM3`Q, -j_?TangentM3`Q] :> 
		scale[]^2 ((-2 pertphi[LI[order]]+2*pertphi[LI[order]]^2) metric\[Delta][-i, -j]  + perth[LI[order], -i, -j] + PD[-i]@pertF[LI[order], -j] + PD[-j]@pertF[LI[order], -i] + 2 PD[-i]@PD[-j]@pertE[LI[order]]),
	Detmetricg[] :> -scale[]^8};
If[fill,
	$SVTDecompositionRules[[1]] = Union[$SVTDecompositionRules[[1]], metricrules];
]
...
matterrules = Flatten[{
	...
	MakeRule[{pertvelocitycov[LI[1], i], (pertvelocityvec[LI[1], i] + metric\[Delta][i, j] PD[-j]@pertvelocity[LI[1]]-scale[] metric\[Delta][i, j] PD[-j]@pertS[LI[1]])/scale[]^2}],
	MakeRule[{pertvelocitycov[LI[2], i], (pertvelocityvec[LI[2], i] + metric\[Delta][i, j] PD[-j]@pertvelocity[LI[2]]-scale[] metric\[Delta][i, j] PD[-j]@pertS[LI[2]])/scale[]^2}],
