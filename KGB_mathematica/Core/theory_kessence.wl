...
DefTensorSVT[pertxi[LI[order]], {M1, M3}, PrintAs -> "\[Zeta]", ScalarPertQ->True]
...
EQalphaK = - hubbleC[]^2/scale[]^2 alphaK[]*Mpl^2 +
	...
	(primescalar[]^4*Derivative[0, 2][G2fun][scalar[], X[]])/scale[]^4 // ToCanonical;


EQalphaKK = - hubbleC[]^2*Mpl^2*alphaKK[]/scale[]^2  -
	(primescalar[]^4*Derivative[0, 2][G2fun][scalar[], X[]])/(4*scale[]^4) -
	(primescalar[]^6*Derivative[0, 3][G2fun][scalar[], X[]])/(12*scale[]^6)// ToCanonical;


EQdensityS = -densityS[] -
	G2fun[scalar[], X[]]+
	(primescalar[]^2*Derivative[0, 1][G2fun][scalar[], X[]])/scale[]^2 // ToCanonical;


EQpressureS = - pressureS[] +
	G2fun[scalar[], X[]]// ToCanonical;
