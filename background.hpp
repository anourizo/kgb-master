...
)
{
	...
	+ (cosmo.Omega_rad / a / a)+ (cosmo.Omega_kgb * pow(a,-3.-3. * cosmo.w_kgb)* a * a)));
	#endif
}

double Hconf_F(const double a, const double fourpiG, const cosmology cosmo)
{
  	return sqrt((2. * fourpiG / 3.) * (((cosmo.Omega_cdm + cosmo.Omega_b + bg_ncdm(a, cosmo)) / a) + (cosmo.Omega_Lambda * a * a)
	+ (cosmo.Omega_rad / a / a)+ (cosmo.Omega_kgb * pow(a,-3.-3. * cosmo.w_kgb)* a * a)));
}
...
double Omega_m(const double a, const cosmology cosmo) { return cosmo.Omega_m / (cosmo.Omega_cdm + cosmo.Omega_b + bg_ncdm(a, cosmo) + cosmo.Omega_kgb * pow(a,-3.-3. * cosmo.w_kgb)* a * a * a + cosmo.Omega_Lambda * a * a * a + cosmo.Omega_rad / a); }
//
double Omega_rad(const double a, const cosmology cosmo) { return (cosmo.Omega_rad + (bg_ncdm(a, cosmo) + cosmo.Omega_cdm + cosmo.Omega_b - cosmo.Omega_m) * a) / ((cosmo.Omega_cdm + cosmo.Omega_b + bg_ncdm(a, cosmo)) * a + cosmo.Omega_kgb * pow(a,-3.-3. * cosmo.w_kgb)* a * a * a * a + cosmo.Omega_Lambda * a * a * a * a + cosmo.Omega_rad); }

//Here Omega_Lambda is just Lambda
double Omega_Lambda(const double a, const cosmology cosmo) { return cosmo.Omega_Lambda / ((cosmo.Omega_cdm + cosmo.Omega_b + bg_ncdm(a, cosmo)) / a / a / a + cosmo.Omega_Lambda + cosmo.Omega_kgb * pow(a,-3.-3. * cosmo.w_kgb) + cosmo.Omega_rad / a / a / a / a);}

double Omega_mg(const double a, const cosmology cosmo) { return (cosmo.Omega_Lambda+ cosmo.Omega_kgb * pow(a,-3.-3. * cosmo.w_kgb)) / ((cosmo.Omega_cdm + cosmo.Omega_b + bg_ncdm(a, cosmo)) / a / a / a + cosmo.Omega_Lambda + cosmo.Omega_kgb * pow(a,-3.-3. * cosmo.w_kgb) + cosmo.Omega_rad / a / a / a / a);}
// Omega_mg = (rho_kgb + rho_Lambda)/rho_tot, however in KGB-evolution Omega_lambda is so tiny and is there because of making sure that Omega_k=0 (the Universe is flat).
#endif
...
)
{
	...
	return a*Hc*dHcda;
	#else
  return (2. * fourpiG / (6. * a * a)) * (  -(cosmo.Omega_cdm + cosmo.Omega_b + bg_ncdm(a, cosmo)) * a + 2. * cosmo.Omega_Lambda * a *  a * a * a - 2. * cosmo.Omega_rad - (1. + 3. * cosmo.w_kgb) * cosmo.Omega_kgb * pow( a, 1.-3.* cosmo.w_kgb));
	#endif
}

double Hconf_prime_F(const double a, const double fourpiG, const cosmology cosmo)
{
  return (2. * fourpiG / (6.)) * (  -(cosmo.Omega_cdm + cosmo.Omega_b + bg_ncdm(a, cosmo)) / a + 2. * cosmo.Omega_Lambda * a *  a  - 2. * cosmo.Omega_rad / a /a - (1. + 3. * cosmo.w_kgb) * cosmo.Omega_kgb * pow( a, -1.-3.* cosmo.w_kgb));
}
...
double Hconf_prime_prime(const double a, const double fourpiG,
	#ifdef HAVE_HICLASS_BG
	gsl_spline * H_spline, gsl_interp_accel * acc
	#else
	const cosmology cosmo
...
)
{
	...
	double Ha = gsl_spline_eval(H_spline, a, acc);
	double Hc = Hconf(a, fourpiG, H_spline, acc);
	double Hc_prime = Hconf_prime(a, fourpiG, H_spline, acc);

	double dHda = gsl_spline_eval_deriv(H_spline, a, acc); // dH/da
    double d2Hda2 = gsl_spline_eval_deriv2(H_spline, a, acc); // d2H/da2
	...
	#else
	//TODO
  return (2. * fourpiG / (6. * a * a)) * (  -(cosmo.Omega_cdm + cosmo.Omega_b + bg_ncdm(a, cosmo)) * a + 2. * cosmo.Omega_Lambda * a *  a * a * a - 2. * cosmo.Omega_rad + (1. + 3. * cosmo.w_kgb) *(1. + 3. * cosmo.w_kgb) * cosmo.Omega_kgb * pow( a, 1.-3.* cosmo.w_kgb)) * sqrt((2. * fourpiG / 3.) * (((cosmo.Omega_cdm + cosmo.Omega_b + bg_ncdm(a, cosmo)) / a) + (cosmo.Omega_Lambda * a * a) + (cosmo.Omega_rad / a / a)+ (cosmo.Omega_kgb * pow(a,-3.-3. * cosmo.w_kgb)* a * a)));
	#endif
}



double Hconf_prime_prime_F(const double a, const double fourpiG, const cosmology cosmo)
{
return (2. * fourpiG / (6.)) * (  (cosmo.Omega_cdm + cosmo.Omega_b + bg_ncdm(a, cosmo)) / a + 4. * cosmo.Omega_Lambda * a *  a  + 4. * cosmo.Omega_rad / a /a + (1. + 3. * cosmo.w_kgb) * (1. + 3. * cosmo.w_kgb) * cosmo.Omega_kgb * pow( a, -1.-3.* cosmo.w_kgb))* sqrt((2. * fourpiG / 3.) * (((cosmo.Omega_cdm + cosmo.Omega_b + bg_ncdm(a, cosmo)) / a) + (cosmo.Omega_Lambda * a * a) + (cosmo.Omega_rad / a / a)+ (cosmo.Omega_kgb * pow(a,-3.-3. * cosmo.w_kgb)* a * a)));
}
