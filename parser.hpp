...
{
  #if defined(HAVE_CLASS) || defined(HAVE_HICLASS) || defined(HAVE_HICLASS_BG)
  if(parallel.isRoot())  cout << " \033[1;31merror:\033[0m"  << "\033[1;35m The code is compiled with the CLASS/hiclass interface while in the settings it is requested to run the code using IC generator (kgb) = file!\033[0m" <<endl;
  parallel.abortForce();
  #endif
}
  ...
  if (ic.IC_kgb == 0)
{
  COUT << " initial conditions for kgb fields (pi,zeta) will be computed using CLASS" << endl;
  #ifndef HAVE_CLASS
...
parallel.abortForce();
	...
	if (parseParameter(params, numparam, "parameters_smg", cosmo.x_i, num_alpha_params))
    {
     ...
     {
       if(parallel.isRoot())  cout << " \033[1;31m ERROR: You need to specify modified parameters_smg!  \033[0m"<< endl;
       parallel.abortForce();
     }
	...
	if (parseParameter(params, numparam, "expansion_smg", cosmo.bg_i, num_bg_params))
    {
     ...
     {
       if(parallel.isRoot())  cout << " \033[1;31m ERROR: You need to specify modified expansion_smg!  \033[0m"<< endl;
       parallel.abortForce();
     }

      if (parseParameter(params, numparam, "omega_kgb", cosmo.Omega_kgb) || parseParameter(params, numparam, "Omega_kgb", cosmo.Omega_kgb))
      {
        if(parallel.isRoot())  cout << " \033[1;31m ERROR: You cannot specify Omega_kgb in the code! It's being obtained through the closed relation! \033[0m"<< endl;
        parallel.abortForce();
      }
      // Lambda
	...
	if (parseParameter(params, numparam, "check_bg_file", par_string))
    {
     ...
     {
       if(parallel.isRoot())  cout << " \033[1;31m ERROR: incorrect entry for check_bg_file. (it should be either 'yes' or 'no')!  \033[0m"<< endl;
       parallel.abortForce();
     }
    }
  ...
  // If hiclass is not requested!
	...
	{
    if(parallel.isRoot())  cout << " \033[1;31m ERROR: You need to specify expansion_smg!  \033[0m"<< endl;
    parallel.abortForce();
	}
	if (!parseParameter(params, numparam, "parameters_smg",  cosmo.x_i))
	{
    if(parallel.isRoot())  cout << " \033[1;31m ERROR: You need to specify parameters_smg!  \033[0m"<< endl;
    parallel.abortForce();
	}
