...
//////////////////////////
// loadTransferFunctions_kgb
//////////////////////////
// Description:
//   loads a set of tabulated transfer functions from a file kgb field
//
// Arguments:
//   filename   string containing the path to the template file
//   pi_k   		will point to the gsl_spline which holds the tabulated
//              transfer function for kgb field (memory will be allocated)
...
void loadTransferFunctions_kgb(const char * filename, gsl_spline * & tk_pi_k, gsl_spline * & tk_zeta, const char * qname, const double boxsize, const double h, double Hconf_gev , double HconfClass)
{
	...
	if (parallel.grid_rank()[0] == 0) // read file
	{
		...
		if (tkfile == NULL)
		{
			cerr << " proc#" << parallel.rank() << ": error in loadTransferFunctions_kgb! Unable to open file " << filename << "." << endl;
			parallel.abortForce();
		}
		...
		while (!feof(tkfile) && !ferror(tkfile))
		{
			...
			if (line[MAX_LINESIZE-1] != 0)
			{
				cerr << " proc#" << parallel.rank() << ": error in loadTransferFunctions_kgb! Character limit (" << (MAX_LINESIZE-1) << "/line) exceeded in file " << filename << "." << endl;
				fclose(tkfile);
				parallel.abortForce();
			}
		...
		if (numpoints < 2)
		{
			cerr << " proc#" << parallel.rank() << ": error in loadTransferFunctions_kgb! No valid data found in file " << filename << "." << endl;
			fclose(tkfile);
			parallel.abortForce();
		}
		...
		if (k == NULL || tk_d == NULL || tk_t == NULL)
		{
			cerr << " proc#" << parallel.rank() << ": error in loadTransferFunctions_kgb! Memory error." << endl;
			fclose(tkfile);
			parallel.abortForce();
		}
		...
		if (kcol < 0 || dcol < 0 || tcol < 0)
		{
			cerr << " proc#" << parallel.rank() << ": error in loadTransferFunctions_kgb       ! Unable to identify requested columns!" << endl;
			fclose(tkfile);
			free(k);
			free(tk_d);
			free(tk_t);
		...
		*ptr = '\0';
		...
		{
			cerr << " proc#" << parallel.rank() << ": error in loadTransferFunctions_kgb! Inconsistent columns!" << endl;
			fclose(tkfile);
			free(k);
			free(tk_d);
			free(tk_t);
		...
		while (!feof(tkfile) && !ferror(tkfile))
		{
			...
			if (sscanf(line, format, dummy, dummy+1, dummy+2) == 3 && !feof(tkfile) && !ferror(tkfile))
			{
				if (dummy[kcol] < 0.)
				{
					cerr << " proc#" << parallel.rank() << ": error in loadTransferFunctions_kgb! Negative k-value encountered." << endl;
					free(k);
					free(tk_d);
					free(tk_t);
					fclose(tkfile);
				...
				if (i > 0)
				{
					if (k[i-1] >= dummy[kcol] * boxsize)
					{
						cerr << " proc#" << parallel.rank() << ": error in loadTransferFunctions_kgb! k-values are not strictly ordered." << endl;
						free(k);
						free(tk_d);
						free(tk_t);
						fclose(tkfile);
				...
				// pi_kgb transfer. Since pi has dimension of 1/H we multiply to H_class/H_Gev
		...
		if (i != numpoints)
		{
			cerr << " proc#" << parallel.rank() << ": error in loadTransferFunctions_kgb.! File may have changed or file pointer corrupted." << endl;
			free(k);
			free(tk_d);
			free(tk_t);
			parallel.abortForce();
	...
	else
	{
		...
		if (numpoints < 2)
		{
			cerr << " proc#" << parallel.rank() << ": error in loadTransferFunctions_kgb! Communication error." << endl;
			parallel.abortForce();
		}
...
double Omega_rad_spl = gsl_spline_eval(rho_rad_spline, a, acc)/gsl_spline_eval(rho_crit_spline, a, acc) + gsl_spline_eval(rho_ur_spline, a, acc)/gsl_spline_eval(rho_crit_spline, a, acc);
...
double HconfClass = Hconf_class( a, cosmo);
	...
	else					// initial displacements and velocities are set by individual transfer functions
	{
      ...
      , HconfClass, Omega_m(a, cosmo), Omega_rad(a, cosmo), Omega_mg(a, cosmo), cosmo.w_kgb
    ...
    else if (ic.tkfile[0] == '\0')
    {
      ...
      loadTransferFunctions(class_background, class_perturbs, tk_d_kgb, tk_t_kgb, "fld", sim.boxsize, sim.z_in, cosmo.h
      // #if defined(HAVE_HICLASS) && !defined(HAVE_HICLASS_BG)
      // , HconfClass, Omega_m(a, cosmo), Omega_rad(a, cosmo), Omega_mg(a, cosmo), cosmo.w_kgb
      // #elif  defined(HAVE_HICLASS) && defined(HAVE_HICLASS_BG)
      // , HconfClass, Omega_m_spl, Omega_smg_spl, Omega_rad_spl, w_smg_spl
    ...
    for (i = 0; i < npts; i++)
    {
      ...
      kgb_field_pi[i] = (HconfClass/Hc) * (tk_t_kgb->y[i] * cosmo.h / sim.boxsize)/(tk_t_kgb->x[i] * cosmo.h/sim.boxsize)/(tk_t_kgb->x[i] * cosmo.h /sim.boxsize);
      kgb_field_pi[i] =  - M_PI * kgb_field_pi[i] * sqrt( Pk_primordial(tk_t_kgb->x[i] * cosmo.h / sim.boxsize, ic)/ tk_t_kgb->x[i])/ tk_t_kgb->x[i];
      ...
      kgb_field_zeta[i] = (cosmo.cs2_kgb/(1.0+cosmo.w_kgb)) * tk_d_kgb->y[i] + 3.0 * cosmo.cs2_kgb *  HconfClass * (tk_t_kgb->y[i] * cosmo.h / sim.boxsize)/(tk_t_kgb->x[i] * cosmo.h/ sim.boxsize)/(tk_t_kgb->x[i] * cosmo.h/ sim.boxsize);

      kgb_field_zeta[i] = - M_PI * kgb_field_zeta[i] * sqrt( Pk_primordial(tk_t_kgb->x[i] * cosmo.h / sim.boxsize, ic)/ tk_t_kgb->x[i])/ tk_t_kgb->x[i];
    ...
    if (ic.IC_kgb == 1)
      {
      if(parallel.isRoot())  cout << " \033[1;31mCAREFUL:\033[0m"  << "\033[1;35m The IC for k-essence is provided using a file! Be careful about the initial conditions for kgb fields! It's safer to use CLASS\033[0m" <<endl;
      loadTransferFunctions_kgb(ic.tk_kgb, tk_d_kgb, tk_t_kgb, "kgb", sim.boxsize, cosmo.h, Hc, HconfClass);	// get transfer functions for kgb
      npts = tk_d_kgb->size;
      kgb_field_pi = (double *) malloc(npts * sizeof(double));
      kgb_field_zeta = (double *) malloc(npts * sizeof(double));
      k_kgb = (double *) malloc(npts * sizeof(double));
      ...
      // K here is in h/Mpc accroding to Pk_primordial(tk_d_kgb->x[i] * cosmo.h / sim.boxsize which is multiplied to h and also we respective Class output notations which is h/Mpc!
      ...
      // Why "-" is here? and where is sqrt(2)?
        kgb_field_pi[i] =  - M_PI * tk_d_kgb->y[i] * sqrt(  Pk_primordial(tk_d_kgb->x[i] * cosmo.h / sim.boxsize, ic)/ tk_d_kgb->x[i])/ tk_d_kgb->x[i];
        // zeta
        kgb_field_zeta[i] = - M_PI * tk_t_kgb->y[i] * sqrt( Pk_primordial(tk_t_kgb->x[i] * cosmo.h / sim.boxsize, ic)/ tk_t_kgb->x[i])
         / tk_t_kgb->x[i];
        k_kgb[i] = tk_d_kgb->x[i];
      }
      // Field realization
